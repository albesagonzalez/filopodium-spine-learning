...
if __name__ == '__main__':
    ...
    plasticity_params["add"] = 0
    plasticity_params["mult"] = 0
    spike_ref_mon, spike_pre_mon, spike_post_mon, w_trajs_FS, post_mon, mu_trajs = run_FS_network(neuron_params, plasticity_params, simulation_params)

    c = patterns[0]["c"]
    w_FS = np.mean(w_trajs_FS[:, -10:], axis=1)
    w_FS = w_FS/np.max(w_FS.flatten())
    filo_index = np.where(w_FS < plasticity_params["w0_minus"])[0]
    ...
    results["c"] = c
    results["w_FS"] = w_FS
    results["filo_index"] = filo_index
    results["spine_index"] = spine_index
    ...
    plasticity_params["mult"] = 0
    spike_ref_mon, spike_pre_mon, spike_post_mon, w_trajs_add, post_mon, mu_trajs = run_FS_network(neuron_params, plasticity_params, simulation_params)



    plasticity_params["add"] = 0
    plasticity_params["mult"] = 1
    spike_ref_mon, spike_pre_mon, spike_post_mon, w_trajs_mult, post_mon, mu_trajs = run_FS_network(neuron_params, plasticity_params, simulation_params)


    results = {}
    results["w_trajs_FS"] = w_trajs_FS
    results["w_trajs_add"] = w_trajs_add
    results["w_trajs_mult"] = w_trajs_mult
